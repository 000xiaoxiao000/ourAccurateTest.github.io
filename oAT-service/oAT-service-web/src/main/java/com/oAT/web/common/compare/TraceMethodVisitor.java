...
//        p.visitLocalVariable(name, desc, signature, start, end, index);
    ...
    public void visitLineNumber(final int line, final Label start) {
        if (compareMethod != null) {
            if (compareMethod.getFirstLine() == -1 || line < compareMethod.getFirstLine()) {
                compareMethod.setFirstLine(line);
            }
            if (line > compareMethod.getLastLine()) {
                compareMethod.setLastLine(line);
            }
        }
