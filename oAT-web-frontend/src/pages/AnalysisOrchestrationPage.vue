...
const loading=ref(false),busy=ref(false),error=ref(''),baselines=ref<VerificationBaseline[]>([]),baselineId=ref(String(route.query.baselineId||'')),job=ref<AnalysisJob|null>(null),graph=ref<GraphSummary|null>(null),symbols=ref(''),result=ref(''),changeType=ref<DiffChangeType>('CODE'),running=ref(''),done=ref<string[]>([])
