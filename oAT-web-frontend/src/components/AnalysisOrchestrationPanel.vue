...
async function incremental(){if(busy.value||!baselineId.value)return;busy.value=true;result.value='';try{let list:string[]=[];if(changeType.value==='CODE')list=await resolveGitAffectedSymbols();const d=await applyDiffInvalidation(props.projectId,baselineId.value,changeType.value);const r=list.length?await incrementalRecompute(props.projectId,baselineId.value,list):null;result.value=`${typeText(d.changeType)}：失效快照 ${d.invalidatedSnapshotKinds.join('、')||'无'}；${r?`自动识别符号 ${list.length} 个，受影响节点 ${r.affectedNodeCount}，AC ${r.affectedAcIds.length}，用例 ${r.affectedTestcaseIds.length}。`:'已完成失效传播。'}`;toast.success('增量分析完成');await loadState()}catch(e){toast.error(msg(e))}finally{running.value='';busy.value=false}}
