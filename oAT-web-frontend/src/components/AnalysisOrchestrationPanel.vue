...
const loading=ref(false),busy=ref(false),baselines=ref<VerificationBaseline[]>([]),baselineId=ref(props.initialBaselineId||''),job=ref<AnalysisJob|null>(null),graph=ref<GraphSummary|null>(null),headCommit=ref(''),result=ref(''),changeType=ref<DiffChangeType>('CODE'),running=ref(''),done=ref<string[]>([])
