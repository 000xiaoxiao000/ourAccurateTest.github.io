...
const backTarget = computed(() => {
  ...
  const routeBackTargets: Record<string, string> = {
    ...
    'graph-explorer': `/p/${projectId.value}/verification`,
...
const routeLabels: Record<string, string> = {
  ...
  'version-compare': '版本比对（旧）',
  'version-report-detail': '比对报告（旧）',
  'map-home': '链路地图',
  'map-app': '源码关系图（旧）',
  'map-code': '代码地图（旧）',
  'search-center': '搜索中心',
...
const routeParentChain: Record<string, Array<{ label: string; to: string }>> = {
  'version-list': [{ label: '源码快照（旧）', to: `/p/${projectId.value}/version/apps` }],
  'version-create': [
    { label: '源码快照（旧）', to: `/p/${projectId.value}/version/apps` },
    { label: '版本列表（旧）', to: `/p/${projectId.value}/apps/${appId.value}/versions` },
  ],
  'version-compare': [
    { label: '源码快照（旧）', to: `/p/${projectId.value}/version/apps` },
    { label: '版本列表（旧）', to: `/p/${projectId.value}/apps/${appId.value}/versions` },
...
const items = computed<BreadcrumbItem[]>(() => {
  ...
  if (name in routeParentChain) {
    ...
    result.push({ label: currentApp.value?.name || '源码工程', to: `/p/${projectId.value}/apps/${appId.value}/settings` })
  }
