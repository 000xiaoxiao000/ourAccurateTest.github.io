...
| 状态 | 判定 |
|---|---|
| `SATISFIED` | 用例、实现和项目要求的执行证据完整且一致 |
...
`SATISFIED` 所需最低证据等级由项目策略决定。没有运行数据时可以形成静态分析结论，但报告必须明确“未包含动态执行证明”。
...
| 通用搜索和通用 AI 助手 | 移出主导航 | 避免稀释 AI 验证主流程 |
...
| 1. 数据与基线 | 建立轻接入和可复现输入 | 文件/Git/一个业务连接器、只读投影、源码索引、Baseline | 无需重复维护即可创建基线 |
| 2. 追溯 | 建立正向和反向关系 | 候选召回、证据、确认、矩阵、失效机制 | 每条关系可解释、可定位、可失效 |
| 3. AI 核心 | 输出可行动的问题 | 用例、代码、交叉分析器，证据校验，结果审核 | 虚假引用为 0，达到人工标注基准集阈值 |
| 4. 回写闭环 | 连接原处理流程 | Bug/任务/评论回写、状态同步、重新验证 | 无重复事项，外部状态可追踪 |
...
6. 平台能区分已满足、部分满足、不满足、歧义、无法验证和过期；
7. AI 无法确认时不会编造资产或事实；
