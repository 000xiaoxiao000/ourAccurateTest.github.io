...
async function runCoverageCommand() {
  ...
  if (r?.success) {
    ...
    if (r.reportDir) { reportDir.value = r.reportDir; reportUrl.value = 'file://' + r.reportDir + '/index.html' }
    msg.value = '指令执行完成'
  } else {
...
function openReport() {
  if (reportUrl.value) window.open(reportUrl.value)
}
...
async function pickClassfiles() {
  const r = await window.electronAPI?.coveragePickPath({ pick: 'dir', title: '选择 classfiles 目录' })
  if (r?.success && r.path) {
...
<template>
  <div class="coverage-page">
    ...
    <div class="card" v-if="selectedBackend && selectedBackend.commands.length">
      ...
      <!-- classfiles 数据源（报告分母）：所有工具通用，与指令表单同源，改这里预览立即变 -->
      <div class="conn-bar">
        <label class="input-group" style="flex:1; min-width:300px">
          <span>classfiles 本地路径<span class="req">*</span></span>
          <span class="path">
            <input :value="cfg().classfilesPath" placeholder="/path/to/target/classes" @input="store.saveCoverageConfig({ classfilesPath: ($event.target as HTMLInputElement).value }); onClassfilesChange(); refreshCommandPreview()" />
            <button class="btn btn-outline btn-sm" @click="pickClassfiles">选择</button>
          </span>
        </label>
        <span class="muted" style="font-size:12px">报告分母字节码/源映射，仅支持本地路径；报告由所选工具原生生成，oAT 内嵌展示</span>
      </div>

      <!-- Agent 连接配置（jacoco 才有远程采集）：与指令表单同源，改这里预览立即变 -->
      <div v-if="selectedBackend.id === 'jacoco'" class="conn-bar">
