...
`oAT-traffic-capture` 是 oAccurateTest 的桌面流量采集器，基于 Electron + Vue 3 构建。它用于测试过程中捕获、过滤、查看、重放和导出 HTTP/HTTPS、WebSocket、MQTT 等流量，也可作为前端和多语言覆盖率的本地上送中继。
...
- 覆盖率中继：内置 `oat-coverage-relay` 插件，可转发 Istanbul、Go、Python、C/C++ 覆盖率。
...
在“覆盖率中心”中配置 oAT 服务地址、项目 ID、应用 ID 和上送间隔，并启用 `oat-coverage-relay` 插件后，采集器会转发到：
...
如果采集器无法直连 `oAT-service-web`，将服务地址配置为 `oAT-relay`，例如 `http://127.0.0.1:18089`。
